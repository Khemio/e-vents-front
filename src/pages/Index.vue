...
export default {
  ...
  methods: {
    ...
    showEventsByType(val) {
      this.events = this.$page.events.edges.filter(edge => edge.node.category === val)
    },
    formatDate(date) {
      return moment(date).format('MMMM Do YYYY, h:mm a')
    },
    getEvents(searchText) {
      return this.events.filter( edge => edge.node.title.toLowerCase().includes(searchText.toLowerCase()));
    }
  }
}
</script>
